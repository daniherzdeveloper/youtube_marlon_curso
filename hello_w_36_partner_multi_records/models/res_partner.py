...
class ResPartner(models.Model):
    ...
    def _compute_hide_btn_phones(self):
        for partner in self:
            ...
            if self.env.user.has_group('hello_w_36_partner_multi_records.group_manager_phones'):
                hide_btn_phones = False
            elif self.env.user.has_group('hello_w_36_partner_multi_records.group_user_phones') and partner.id == self.env.user.partner_id.id:
                    hide_btn_phones = False
    ...
    def action_view_partner_phones(self):
        action = self.env.ref('hello_w_36_partner_multi_records.partner_phone_action').sudo().read()[0]
