...
class CreateInvoice(models.TransientModel):
    ...
    def action_create_invoice(self):
        ...
        if not self.product_id:
            raise UserError(_("Debe seleccionar un producto para crear la factura."))

        invoice_vals = self._prepare_invoice_vals()
        invoice_line_vals = self._prepare_invoice_line_vals()

        if not invoice_line_vals.get('product_id'):
            raise UserError(_("El producto en la línea de factura no es válido."))

        invoice_vals.update({'invoice_line_ids': [(0, 0, invoice_line_vals)]})
